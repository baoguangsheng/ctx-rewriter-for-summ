...
def run(args, device_id, error_queue):
    ...
    try:
        ...
        import traceback
        print("gpu_rank %s exception: %s" % (args.gpu_ranks[device_id], traceback.format_exc()))
    ...
    if (args.test_all):
        ...
        for i, cp in enumerate(cp_files):
            ...
            if (args.test_start_from != -1 and step < args.test_start_from):
                acc_xent_lst.append((-1, 1e6, cp))
                continue
            xent, acc = validate(args, device_id, cp, step)
            acc_xent_lst.append((acc, xent, cp))
            max_step = acc_xent_lst.index(max(acc_xent_lst))  # select the maximum of acc
            if (i - max_step > 10):
                break
        acc_xent_lst = sorted(acc_xent_lst, key=lambda x: x[0], reverse=True)[:3]
        logger.info('acc/xent/checkpoint %s' % str(acc_xent_lst))
        # for acc, xent, cp in acc_xent_lst:
        #     step = int(cp.split('.')[-2].split('_')[-1])
...
def validate(args, device_id, pt, step):
    ...
    valid_loss = abs_loss(model.generator, symbols['PAD'], model.vocab_size, train=False, device=device)
...
def test_abs(args, device_id, pt, step):
    ...
    symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}
    predictor = build_predictor(args, tokenizer, symbols['PAD'], model, logger)
...
def train_abs_single(args, device_id):
    ...
    tokenizer = BertTokenizer.from_pretrained(model.bert.model_name, do_lower_case=True, cache_dir=args.temp_dir)
    symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}
